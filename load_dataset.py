...
def load_w2v_dataset(filename="data/raw/train.csv", text_max_length=50, target_max_length=1, task="acp",
...
def load_alberto_dataset(filename="data/raw/train.csv", text_max_length=50, target_max_length=1, task="acd",
                         word_indices=None, split_validation=True):
    ...
    final_reviews = []
    for elem in x_train:
        final_topics.append(elem[0])  # list of np.array(1,768)
        final_reviews.append(elem[1])  # list of np.array(50, 768)
    x_train = [np.array(final_topics), np.array(final_reviews)]
    y_train = np.array(categories_to_onehot(y_train))
